...
const Step10 = ({ prevStep, nextStep, handleFormData, values }) => {
  ...
  const handlInputData2 = (input) => (e) => {
    ...
    values.obiettivo_f = e.target.name;
  };
  ...
  const submitFormData = (e) => {
    ...
    if (validator.isEmpty(values.obiettivo_f)) {
      setError(true);
    } else {
      nextStep();
    }
        ...
        <Card className={cn("card", styles.card)}>
          <Card.Body>
            ...
            <div className={cn("title", styles.title)}>
              <div className={cn("number", styles.number)}>{step + 13}</div>
                ...
                {["radio"].map((type) => (
                  ...
                  >
                    <Form.Check
                      ...
                      onChange={handlInputData2("obiettivo_f")}
                    />
                    <Form.Check
                      type={type}
                      id={`default-${type}`}
